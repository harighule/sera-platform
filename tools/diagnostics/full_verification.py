...
for seed in NEW_SEEDS:
    ...
    print(
        f"  [Seed {seed}] Done -- synthetic_val_loss={val_loss:.4f}  "
        f"synthetic_self_consistency_acc={val_acc*100:.1f}%  metric_type={metric_type}",
        flush=True,
    )
...
print(f"\n{'Seed':<8} {'Synthetic Acc':<15} {'Val Loss':<10} {'>=60%?'}")
print(SEP2)
for s in NEW_SEEDS:
    r = seed_results[s]
